...
public class WBS_7c {
	...
	public void update(int PedalPos, boolean AutoBrake, boolean Skid) {
		...
		if ((PedalPos == 0)) {
			...
			if (change(PedalPos == 1, PedalPos <= 1)) {
		...
		if ((WBS_Node_WBS_BSCU_Switch2 == 0)) {
			Nor_Pressure = change(0, Nor_Pressure);
		} else {
		...
		if ((WBS_Node_WBS_BSCU_Command_PedalCommand_Switch1 == 0)) {
			...
			if (change(WBS_Node_WBS_BSCU_Command_PedalCommand_Switch1 == 1, false)) {
