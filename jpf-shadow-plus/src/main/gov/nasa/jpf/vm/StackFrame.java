 ...
 *
 *
 * The original implementation of this class belongs to the jpf-core project. 
 * It is being extended to use for the jpf-shadow extension.
 */

public abstract class StackFrame implements Cloneable {

	 //--- extended implementation starting here
	  public void storeLongOperand(int index, ThreadInfo ti, Instruction insn){
		  	/*
		  	 * stores value from the top slot in the index slot
		  	 * long operands occupy two consecutive slots
		  	 * top value is stored in slots[top-1] and slots[top] 
		  	 * and will be stored into slots[index],slots[index+1]
		  	 * 
		  	...
		  	Expression shadow_result = null;
		  	Expression sym_result = null;
		  	...
		  	if(attrs!= null){
		  		switch(ti.getExecutionMode()){
		  			...
		  			case OLD: //we only want to store the old (shadow) symbolic expression
		  				...
		  				if(attrs[index] != null){
		  					if(attrs[index] instanceof DiffExpression){
		  						sym_result = ((DiffExpression) attrs[index]).getSymbc();
		  					}
		  					else{
		  						sym_result = (Expression) attrs[index];
		  					}
		  				}
		  				else{//index value is concrete
		  					if(insn instanceof LSTORE){
		  						long concreteValue = this.getLongLocalVariable(index);
		  						sym_result = new IntegerConstant((int) concreteValue);
		  					}
		  					else{
		  						assert(insn instanceof DSTORE);
		  						double concreteValue = this.getDoubleLocalVariable(index);
		  						sym_result = new RealConstant(concreteValue);
		  					}
		  				}
		  				break;
		  			...
		  			case NEW: //we only want to store the new symbolic expression
		  				...
		  				if(attrs[t] != null){
		  					if(attrs[t] instanceof DiffExpression){
		  						sym_result = ((DiffExpression)attrs[t]).getSymbc();
		  					}
		  					else{
		  						sym_result = (Expression) attrs[t];
		  					}
		  				}
		  				...
		  				if(attrs[index] != null){
		  					if(attrs[index] instanceof DiffExpression){
		  						shadow_result = ((DiffExpression)attrs[index]).getShadow();
		  					}
		  					else{
		  						shadow_result = (Expression) attrs[index];
		  					}
		  				}
		  				else{
		  					if(insn instanceof LSTORE){
		  		...
		  		//TODO: we might want to create a diffexpression here if the executionmode is old or new
		  	}
	  ...
	  public void storeOperand(int index, ThreadInfo ti, Instruction insn){
		  ...
		  if(attrs != null){
			  /*
			   * In the cases OLD and NEW we only want to change the 
			   * shadow expression or the symbolic expression, respectively.
			   * Therefore, the object we store as the attribute value will always be a DiffExpression.
			   * However, the case BOTH will simply store the attribute value of the top slot (not necessarily a DiffExpr).
			   */
			  switch(ti.getExecutionMode()){
			  	case BOTH:
			  	...
			  	case OLD: //we only want to modify the symbolic expression
			  		...
			  		if(attrs[top]!=null){
			  			if(attrs[top] instanceof DiffExpression){
			  				shadow_result = ((DiffExpression) attrs[top]).getShadow();
			  			}
			  			else{
			  				shadow_result = (Expression) attrs[top];
			  			}
			  		}
			  		else{ 
						if(insn instanceof ISTORE){
							shadow_result = new IntegerConstant(slots[top]); 
						}
						else{
							assert(insn instanceof FSTORE);
							shadow_result = new RealConstant(this.getFloatLocalVariable(top));
						}
					}
					//save the current shadow expression from the index slot since we dont want to modify it
					if(attrs[index]!=null){
						if(attrs[index] instanceof DiffExpression){
							sym_result = ((DiffExpression) attrs[index]).getSymbc();
					  	}
						...
						if(insn instanceof ISTORE){
							sym_result = new IntegerConstant(slots[index]);
						}
						else{
							assert(insn instanceof FSTORE);
							sym_result = new RealConstant(this.getFloatLocalVariable(index));
						}
					}
			  		break;
			  	case NEW:
			  		//we only want to modify the shadow expression, therefore we only need the new shadow expression
			  		if(attrs[top]!=null){
			  			if(attrs[top] instanceof DiffExpression){
			  				sym_result = ((DiffExpression) attrs[top]).getSymbc();
			  			}
			  			else{
			  				sym_result = (Expression) attrs[top];
			  			}
			  		}				
			  		else{
						if(insn instanceof ISTORE){
							sym_result = new IntegerConstant(slots[top]);
						}
						else{
							assert(insn instanceof FSTORE);
							sym_result = new RealConstant(this.getFloatLocalVariable(top));
						}
					}
					...
					if(attrs[index]!=null){
						...
						if(insn instanceof ISTORE){
							shadow_result = new IntegerConstant(slots[index]);
						}
						else{
							assert(insn instanceof FSTORE);
							shadow_result = new RealConstant(this.getFloatLocalVariable(index));
						}	
					}
