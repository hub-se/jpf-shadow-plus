		...
		if(op_v1==null && op_v2==null&& th.getExecutionMode() == Execute.BOTH){
			...
			if(th.getExecutionMode() ==Execute.OLD || th.getExecutionMode() ==Execute.BOTH){
				...
				//add the symbolic expressions
		  		if(shadow_v1!=null) {
		  			if (shadow_v2!=null){
		  				shadow_result = shadow_v1._xor(shadow_v2);
		  			}
					else{ // v2 is concrete
						shadow_result = shadow_v1._xor(v2);
					}
				}
				else if (shadow_v2!=null){
						shadow_result = shadow_v2._xor(v1);
				}
			}
			...
			if(th.getExecutionMode() ==Execute.NEW || th.getExecutionMode() ==Execute.BOTH){
				...
				if(sym_v1!=null) {
		  			if (sym_v2!=null){
		  				sym_result = sym_v1._xor(sym_v2);
		  			}
					else{ // v2 is concrete
						sym_result = sym_v1._xor(v2);
					}
				}
				else if (sym_v2!=null){
						sym_result = sym_v2._xor(v1);
				}
			}
