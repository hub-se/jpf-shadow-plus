...
public class SymbolicListener extends PropertyListenerAdapter implements PublisherExtension {
	...
	public void propertyViolated(Search search) {
		...
		if ((cg instanceof PCChoiceGenerator) && ((PCChoiceGenerator) cg).getCurrentPC() != null) {
			...
			//jpf-shadow: only generate test cases for paths exposing a divergence
			if(!pc.isDiffPC()) {
				return;
			}
	...
	public void instructionExecuted(VM vm, ThreadInfo currentThread, Instruction nextInstruction,
		...
		if (!vm.getSystemState().isIgnored()) {
			...
			if (insn instanceof JVMInvokeInstruction) {
				...
				//jpf-shadow: handle change(boolean,boolean), TODO Why is this necessary? Would this generate duplicate test cases?
				if(!ti.isFirstStepInsn() && ((JVMInvokeInstruction)insn).getInvokedMethodName().endsWith("change(ZZ)Z")){
					return;
				}
				...
				if (null != ci) {
					...
					if (((BytecodeUtils.isClassSymbolic(conf, className, mi, methodName))
						...
						//jpf-shadow: generate test case iff the execution path contains a divergence, i.e. is a diff path				
	...
	private void printMethodSummary(PrintWriter pw, MethodSummary methodSummary) {
		...
		if (pathConditions.size() > 0) {
			...
			while (it.hasNext()) {
				...
				//jpf-shadowSym, patch: latest jpf-symbc did not properly output the test inputs
				...
				//since the bytecode executes the negated if-insn, we might need to switch diffTypes
